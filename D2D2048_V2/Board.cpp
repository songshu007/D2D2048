...
int Board::aiDullScore(int** arr, int rows, int cols)
{
	// 对棋盘的单调性进行评分
	int score = 0;
	// 横
	for (int y = 0; y < rows; y++)
	{
		for (int x = 0; x < cols - 1; x++)
		{
			if ((arr[y][x] > arr[y][x + 1]) || (arr[y][x] < arr[y][x + 1])) score++;
		}
	}
	// 竖
	for (int x = 0; x < cols; x++)
	{
		...
		{
			if ((arr[y][x] > arr[y + 1][x]) || (arr[y][x] < arr[y + 1][x])) score++;
		}
	}
	...
	for (int i = 0; i < _deep; i++)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			...
			while (!isGameOver(temp_board, rows, cols))
			{
				...
				float m_s = 0.0f, z_s = 0.0f;
				m_s = move((DIR)(rand() % 4), temp_board, rows, cols);
				z_s = getArrZeroNum(temp_board, rows, cols);
