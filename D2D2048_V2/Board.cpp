...
void Board::Render(app::Direct2dApp& App, RECT& _rc)
{
	...
	case SIZEMOVE:
	{
		...
		for (auto it = m_add_animation.begin(); it != m_add_animation.end(); it++)
		{
			App.getDC()->SetTransform(D2D1::Matrix3x2F::Translation(
				boardX + m_spacing + it->x * (BlockWidth + m_spacing), boardY + m_spacing + it->y * (BlockWidth + m_spacing)
			));

			App.fillRoundedRect(0.0f, 0.0f, BlockWidth, BlockWidth, BlockWidth * 0.1f, BlockWidth * 0.1f, D2D1::ColorF(187 / 255., 173 / 255., 160 / 255.));
			App.fillRoundedRect(0.0f, 0.0f, BlockWidth, BlockWidth, BlockWidth * 0.1f, BlockWidth * 0.1f, getColor(0));

			App.fillRoundedRect((BlockWidth - BlockWidth * movePercent) * 0.5f, (BlockWidth - BlockWidth * movePercent) * 0.5f, BlockWidth* movePercent, BlockWidth* movePercent, BlockWidth * 0.1f, BlockWidth * 0.1f, this->getColor(this->m_board[it->y][it->x]));

			wsprintfW(buf, L"%d", (int)pow(2, this->m_board[it->y][it->x]));
			App.drawCenterText(buf, { 0.0f, 0.0f }, BlockWidth, BlockWidth, getTextColor(this->m_board[it->y][it->x]), BlockWidth* getTextSize(this->m_board[it->y][it->x])* movePercent);
