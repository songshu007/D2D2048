...
INT_PTR  AppWindow::WindowDiglogProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_COMMAND:
...
INT_PTR  AppWindow::WindowDiglogAboutProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_INITDIALOG:
	{
		auto bmp = LoadImageW(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_BITMAP1), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR);
		SendDlgItemMessage(hwnd, IDC_PICTURE, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)bmp);
	}
	break;
	case WM_COMMAND:
	{
...
LRESULT AppWindow::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case ID_BOARD_SIZE:
		{
			DialogBoxW(NULL, (LPCWSTR)IDD_DIALOG1, hwnd, WindowDiglogProc);
			sInstance->m_board.ReSize(sInstance->m_board_width, sInstance->m_board_height);
		}
		break;
		case ID_ABOUT:
		{
			DialogBoxW(NULL, (LPCWSTR)IDD_DIALOG2, hwnd, WindowDiglogAboutProc);
		}
		break;
		case ID_Vertical:
		{
		...
		case ID_LINE:
		{
			sInstance->m_board.setAnimationMode(LINE);
		}
		break;
		case ID_QUAD:
		{
			sInstance->m_board.setAnimationMode(QUAD);
		}
		break;
		case ID_MAX_QUAD:
		{
			sInstance->m_board.setAnimationMode(MAX_QUAD);
		}
		break;
		case ID_JELLY:
		{
			sInstance->m_board.setAnimationMode(JELLY);
		}
		break;
		case ID_FALL:
		{
			sInstance->m_board.setAnimationMode(FALL);
		}
		break;
		case ID_SPEED_NORMAL:
		{
			sInstance->m_board.setAnimationSpeed(NORMAL);
		}
		break;
		case ID_SPEED_MEDIUM:
		{
			sInstance->m_board.setAnimationSpeed(MEDIUM);
		}
		break;
		case ID_SPEED_SLOW:
		{
			sInstance->m_board.setAnimationSpeed(SLOW);
		}
		break;
		default:
			break;
	...
	HMENU h = GetMenu(hwnd);
	CheckMenuItem(h, ID_LINE, sInstance->m_board.m_animation_mode == LINE ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(h, ID_QUAD, sInstance->m_board.m_animation_mode == QUAD ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(h, ID_MAX_QUAD, sInstance->m_board.m_animation_mode == MAX_QUAD ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(h, ID_JELLY, sInstance->m_board.m_animation_mode == JELLY ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(h, ID_FALL, sInstance->m_board.m_animation_mode == FALL ? MF_CHECKED : MF_UNCHECKED);

	CheckMenuItem(h, ID_SPEED_NORMAL, sInstance->m_board.m_animation_speed_mode == NORMAL ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(h, ID_SPEED_MEDIUM, sInstance->m_board.m_animation_speed_mode == MEDIUM ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(h, ID_SPEED_SLOW, sInstance->m_board.m_animation_speed_mode == SLOW ? MF_CHECKED : MF_UNCHECKED);
